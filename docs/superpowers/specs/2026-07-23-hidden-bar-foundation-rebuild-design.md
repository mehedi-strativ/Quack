...
**Date:** 2026-07-23
**Status:** Removed (2026-07-24). Implemented, then manual QA on real hardware
surfaced a divider-drift bug not anticipated by this design (every
`expand()`/`collapse()` cycle permanently shifts the divider's position) —
fixed post-merge (see `git log --grep drift`). A separate, pre-existing
glyph-capture bug (hidden items always render their generic app icon instead
of the real menu-bar glyph) was found during the same QA pass, confirmed
unrelated. After all that, the user decided to drop the whole feature rather
than keep chasing hidden-menu-bar edge cases — all code deleted (see
`git log --grep "drop.*hidden"` or the commit right after this doc's last
update for the removal). The Roadmap below (phases 2-6) is moot.
